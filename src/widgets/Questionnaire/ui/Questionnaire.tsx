...
const googleFormUrl =
  "https://docs.google.com/forms/d/e/1FAIpQLSf9EovzzWxsiJDx7nZ9p_5tpvvbhz5-Z6bOBzZSvN3nLdSqAg";
...
const Questionnaire: FC = () => {
  ...
  const validate = () => {
    ...
    if (!form.secondName.trim()) newErrors.secondName = "Фамилия обязательна";
    if (!form.isConfirm) newErrors.isConfirm = "Необходимо подтвердить участие";
    return newErrors;
  };
  ...
  const handleSubmit = async () => {
    ...
    form.alcohol.forEach((id) => {
      ...
      if (label) {
        formData.append("entry.162603456", label);
      }
    });
    ...
    try {
      await fetch(`${googleFormUrl}/formResponse`, {
        method: "POST",
        mode: "no-cors",
        body: formData,
      });
